...
class Api_model extends CI_Model
{
	...
	function all_categoria_get(){
		$all_categoria = array();
		$this->db->where('parent', 0);
		$categoria = $this->db->get('category')->result_array();
		foreach ($categoria as $key => $category) {
			$all_categoria[$key] = $category;

			$this->db->where('parent', $category['id']);
			$sub_categoria = $this->db->get('category')->result_array();
			$all_categoria[$key]['sub_categoria'] = $sub_categoria;
		}
	...
	// Get categoria
	public function categoria_get($category_id)
	{
		if ($category_id != "") {
		...
		$categoria = $this->db->get('category')->result_array();
		foreach ($categoria as $key => $category) {
			$categoria[$key]['thumbnail'] = $this->get_image('category_thumbnail', $category['thumbnail']);
			$categoria[$key]['number_of_courses'] = $this->crud_model->get_category_wise_courses($category['id'])->num_rows();

			$this->db->where('parent', $category['id']);
			$number_of_sub_categoria = $this->db->get('category')->num_rows();
			$categoria[$key]['number_of_sub_categoria'] = $number_of_sub_categoria;
		}
		return $categoria;
	}

	// Get sub categoria
	public function sub_categoria_get($parent_category_id)
	{
		$response = array();
	...
	// Get image for course, categoria or user image
